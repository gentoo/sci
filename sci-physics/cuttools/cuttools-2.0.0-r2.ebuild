...
src_compile() {
	filter-lto
	if use mpfun90; then
		emake -j1 FFLAGS="${FFLAGS} -I${ESYSROOT}/usr/include/ -I${ESYSROOT}/usr/include/mpfun90 -fPIC -std=legacy" mp
	else
		emake -j1 FFLAGS="${FFLAGS} -I${ESYSROOT}/usr/include/ -I${ESYSROOT}/usr/include/mpfun90 -fPIC -std=legacy" qp
	fi
	tc-export AR CXX
	cd includects || die
	${AR} -x libcts.a || die
