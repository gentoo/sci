...
src_install() {
	mpi_dobin bin/gentoo_hpl_fblas_x86/xhpl || die
	mpi_dolib.a lib/gentoo_hpl_fblas_x86/libhpl.a || die
	mpi_dodoc INSTALL BUGS COPYRIGHT HISTORY README TUNING \
		bin/gentoo_hpl_fblas_x86/HPL.dat || die
	mpi_doman man/man3/*.3
	if use doc; then
		mpi_dohtml -r www/* || die
	fi
}
...
pkg_postinst() {
	...
	einfo "Typically one may run linpack by executing this in your working directory"
