...
src_unpack() {
	...
	epatch ${FILESDIR}/${PN}-2.0.0_p5-respect-ldflags.patch || die "Failed to fix ldflags"
	epatch ${FILESDIR}/${PN}-2.0.0_p7-respect-destdir.patch || die "Failed to fix Makefiles for DESTDIR"
	epatch ${FILESDIR}/${PN}-2.0.0_p5-destdir-fixes.patch || die "Failed to fix DESTDIR for gui"
	epatch ${FILESDIR}/${PN}-2.0.0_p5-setuid-safety.patch || die "Failed to fix setXid linking for pbs_rcp"
	epatch ${FILESDIR}/${PN}-2.0.0_p5-makedepend.patch || die "Failed to fix buildutils/makedepend-sh"
